...
class GameData {
    constructor(update_interval, enable_update) {
        ...
        this.csgo_api_url = 'https://raw.githubusercontent.com/ByMykel/CSGO-API/main/public/api/en/skins.json';

        this.items_game = false;
        this.items_game_cdn = false;
        this.csgo_english = false;
        this.schema = false;
        this.csgo_api_skins = {}; // Lookup by defindex_paintindex
    ...
    */
    loadFiles() {
        ...
        if (fs.existsSync('game_files/csgo_api_skins.json')) {
            let data = fs.readFileSync('game_files/csgo_api_skins.json', 'utf8');
            this.buildSkinLookup(JSON.parse(data));
        }
    }

    /*
        Builds a lookup table for CSGO-API skins by defindex_paintindex
    */
    buildSkinLookup(skins) {
        for (const skin of skins) {
            const key = `${skin.weapon.weapon_id}_${skin.paint_index}`;
            this.csgo_api_skins[key] = skin.image;
        }
        winston.debug(`Built CSGO-API skin lookup with ${Object.keys(this.csgo_api_skins).length} entries`);
    }
    ...
    */
    update() {
        ...
        utils.downloadFile(this.csgo_api_url, (data) => {
            if (data) {
                winston.debug('Fetched CSGO-API skins.json');
                const skins = JSON.parse(data);
                this.buildSkinLookup(skins);
                fs.writeFileSync('game_files/csgo_api_skins.json', data, 'utf8');
            }
            else winston.error('Failed to fetch CSGO-API skins.json');
        });
    }
    ...
    */
    addAdditionalItemProperties(iteminfo) {
        ...
        if (image_name in this.items_game_cdn) {
            ...
            // Fallback 1: Try CSGO-API lookup by defindex_paintindex
            const lookup_key = `${iteminfo.defindex}_${iteminfo.paintindex}`;
            if (lookup_key in this.csgo_api_skins) {
                iteminfo['imageurl'] = this.csgo_api_skins[lookup_key];
            } else if (weapon_name && skin_name) {
                // Fallback 2: Generate CDN URL (deprecated, may not work)
                const cdn_image_name = image_name + '_light_large.png';
                iteminfo['imageurl'] = `https://steamcdn-a.akamaihd.net/apps/730/icons/econ/default_generated/${cdn_image_name}`;
            }
        }
