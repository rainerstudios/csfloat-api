...
async function addInventoryEnhancement(itemInfoContent, floatData, itemName, inspectLink) {
  ...
  // Add blue gem info if applicable
  if (floatData.blueGemInfo) {
    enhancementHtml += `
      <div style="background: rgba(0, 100, 255, 0.15); padding: 8px; border-radius: 4px; margin-bottom: 8px;">
        <strong style="color: #4da6ff;">💎 Blue Gem Analysis</strong><br>
        <div style="margin-top: 4px;">
          Blue Coverage: <strong>${floatData.blueGemInfo.bluePercentage}%</strong> |
          Tier: <strong>${floatData.blueGemInfo.tier}</strong><br>
          Est. Value: $${floatData.blueGemInfo.estimatedValue.min} - $${floatData.blueGemInfo.estimatedValue.max}
        </div>
      </div>
    `;
  }
