...
class Postgres {
    ...
    /**
     * Cache price data to reduce API calls (for 300 concurrent users)
     * @param {string} marketHashName - Item market hash name
     * @param {object} priceData - Price data from API
     * @returns {Promise<void>}
     */
    async cachePriceData(marketHashName, priceData) {
        await this.pool.query(`
            INSERT INTO price_cache (market_hash_name, price_data, cached_at)
            VALUES ($1, $2, NOW())
            ON CONFLICT (market_hash_name)
            DO UPDATE SET price_data = $2, cached_at = NOW()
        `, [marketHashName, JSON.stringify(priceData)]);
    }

    /**
     * Get cached price data (expires after 5 minutes)
     * @param {string} marketHashName - Item market hash name
     * @returns {Promise<Object|null>} Cached price data or null
     */
    async getCachedPrice(marketHashName) {
        const result = await this.pool.query(`
            SELECT price_data, cached_at
            FROM price_cache
            WHERE market_hash_name = $1
              AND cached_at > NOW() - INTERVAL '5 minutes'
        `, [marketHashName]);

        return result.rows.length > 0 ? result.rows[0].price_data : null;
    }

    /**
     * Store daily price snapshot
     * @param {string} marketHashName - Item market hash name
     * @param {string} market - Market name (buff163, skinport, etc)
     * @param {number} priceUsd - Price in USD
     * @param {number} listings - Number of listings
     * @param {object} options - Additional options
     * @returns {Promise<void>}
     */
    async storePriceSnapshot(marketHashName, market, priceUsd, listings, options = {}) {
        await this.pool.query(`
            INSERT INTO item_prices (
                market_hash_name, market, price_usd,
                price_original, currency, listings, date
            )
            VALUES ($1, $2, $3, $4, $5, $6, CURRENT_DATE)
            ON CONFLICT (market_hash_name, market, date)
            DO UPDATE SET
                price_usd = $3,
                listings = $6,
                updated_at = NOW()
    ...
    /**
     * Store recent sale data
     * @param {string} marketHashName - Item market hash name
     * @param {object} saleData - Sale data
     * @returns {Promise<void>}
     */
    async storeRecentSale(marketHashName, saleData) {
        await this.pool.query(`
            INSERT INTO recent_sales (
                market_hash_name, price_usd, float_value,
                pattern, stickers, sale_date, market
            )
        ...
        `, [
            ...
            saleData.float ? parseFloat(saleData.float) : null,
            saleData.pattern || null,
            saleData.stickers ? JSON.stringify(saleData.stickers) : null,
            saleData.date,
            saleData.market
        ]);
    }
    ...
    /**
     * Calculate float price premium based on sales data
     * @param {string} marketHashName - Item market hash name
     * @param {number} floatValue - Float value to analyze
     * @returns {Promise<Object>} Price premium analysis
     */
    async calculateFloatPricePremium(marketHashName, floatValue) {
        const result = await this.pool.query(`
            SELECT
        ...
        if (result.rows.length === 0 || result.rows[0].total_sales === 0) {
            return {
                success: false,
        ...
        // Find sales with similar float (±0.05)
        const similarSales = await this.pool.query(`
            SELECT AVG(price_usd) as avg_price, COUNT(*) as count
            FROM recent_sales
            WHERE market_hash_name = $1
              AND float_value IS NOT NULL
              AND ABS(float_value - $2) < 0.05
              AND sale_date > NOW() - INTERVAL '60 days'
        `, [marketHashName, floatValue]);

        const similarAvg = similarSales.rows[0].count > 0
            ? parseFloat(similarSales.rows[0].avg_price)
            : parseFloat(stats.avg_price);

        const premium = ((similarAvg - parseFloat(stats.avg_price)) / parseFloat(stats.avg_price) * 100);

        return {
            success: true,
            yourFloat: floatValue,
            estimatedPrice: similarAvg.toFixed(2),
            marketAverage: parseFloat(stats.avg_price).toFixed(2),
            premiumPercent: premium.toFixed(2),
            sampleSize: parseInt(similarSales.rows[0].count),
            totalSales: parseInt(stats.total_sales),
            priceRange: {
                min: parseFloat(stats.min_price).toFixed(2),
                max: parseFloat(stats.max_price).toFixed(2)
            }
        };
    }
    ...
    /**
     * Get list of unique items to sync prices for
     * @returns {Promise<Array>} List of market hash names
     */
    async getItemsToSync() {
        const result = await this.pool.query(`
            SELECT DISTINCT
                defindex,
                paintindex,
